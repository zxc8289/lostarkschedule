...
import './weeklyGroupCreator.css';
import { useNavigate } from 'react-router-dom';
import RaidSchedule from '../raidCheck/raidCheck';
...
import RaidCheck from '../raidCheck/raidCheck';

function WeeklyGroupCreator() {
    ...
    useEffect(() => {
        const raidSchedulesRef = ref(db, 'RaidCheck');
    ...
    const createGroup = () => {
        ...
        const newRoomKey = push(child(ref(db), 'RaidCheck')).key;

        // 그룹에 참여자 목록 추가
        set(ref(db, 'RaidCheck/' + newRoomKey), {
            title: roomTitle,
            createdBy: userId,
            members: {
    ...
    const joinGroup = () => {
        ...
        const groupRef = ref(db, `RaidCheck/${groupJoinId}/members`);
    ...
    const groupSettingComplete = () => {
        ...
        update(ref(db, `RaidCheck/${selectedId}`), { title: groupTitle })
            .then(() => {
    ...
    return (
        <div className="flex-container">
            ...
            <GroupModal
                show={groupCreationModal}
                onHide={() => groupCreationModalClose()}
                title="그룹 생성"
                subTitle="그룹을 생성하고 레이드를 함께 갈 그룹원을 초대할 수 있어요."
                content="그룹 이름"
            ...
            />
            <GroupModal
                show={groupParticipationModal}
                onHide={() => groupParticipationModalClose()}
                title="그룹 참여"
                subTitle="초대를 받았다면 그룹에 참여할 수 있어요."
                content="초대 링크 입력"
            ...
            <div className="flex-center">
                ...
                <GroupSettingModal
                    ...
                    title="그룹 설정"
                    subtitle="그룹 이름을 변경하거나 수정권한을 부여해보세요."
                    subtitle2="모든 사람에게 수정 권한을 부여할 수 있습니다."
                    textInputName="그룹 이름 변경"
                    textInputName2="수정 권한 부여"
                    placeholder="그룹 이름을 입력하세요."
                />

                {selectedId && <RaidCheck id={selectedId} createdBy={createdBy} />}
            </div>
...
export default WeeklyGroupCreator;
