...
import './raidCheck.css';
import { fetchUserApiKey } from "../../utils/apiKeys";
import CustomModal from "../../componnent/modal/customModal";
import { Modal } from "react-bootstrap";
import MessageModal from "../../componnent/modal/messageModal";


function RaidCheck({ id, createdBy }) {
    ...
    const messageModalClose = () => setMessage(false);
    const messageModalShow = () => setMessage(true);
    ...
    const leaveGroupButton = async (createdBy) => {
        try {
            // 그룹 정보를 가져옵니다.
            const raidScheduleRef = ref(db, `/RaidCheck/${raidScheduleId}`);
            ...
            if (createdBy === auth.currentUser.uid) {
                ...
                if (remainingMembers.length > 0) {
                    ...
                    await update(ref(db, `/RaidCheck/${raidScheduleId}`), { createdBy: newCreatedBy });
                } else {
            ...
            await set(ref(db, `/RaidCheck/${raidScheduleId}/members/${auth.currentUser.uid}`), null);
    ...
    return (
        <div>
            ...
            <MessageModal show={message} onHide={messageModalClose} title="링크가 복사되었습니다." />
            ...
            <CustomModal showModal={addCharacter} handleModalClose={addCharacterModalClose} title="계정 추가" subtitle="레이드 현황을 확인하고 관리할 캐릭터와 계정을 불러올게요." content="" placeholder="불러올 계정의 대표 캐릭터명을 입력해주세요." link={addCharacterTextInput} buttonName="추가" button={addCharacterClick} linkFontSize={14} linkWidth={290} handleInputChange={addCharacterInputChange} type={false} handleKeyDown={handleKeyDown} />

        </div >
    );
}

export default RaidCheck;
